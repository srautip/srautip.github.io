...
| **Stammdaten.vb** | Typisiertes Domänenmodell (Klassenstufe/Fach/Klasse/Raum/Lehrer/FachLehrerZuordnung) für dauerhaft verwaltete Schul-Stammdaten (Phase 2.15) - bewusst NICHT das rohe `JsonObject`-Muster der Constraints (siehe 8.7). Laden/Speichern als JSON, `BuildEntitiesFragment` projiziert in das bestehende `entities`-Format. Phase 2.19 ergänzt ein Mitgliedschaftsdatenmodell (`Schueler`: pseudonyme ID + Heimatklasse; `Gruppe`: benannte, klassenunabhängige Schülergruppe für Fachgruppen/Förderung/Aufsicht) - Schritt 1, rein additiv, noch ohne Solver-/Verifier-Wirkung (siehe `docs/phase2-19-mitgliedschaftsmodell.md`). | - |
| **StammdatenValidation.vb** | Cross-Reference-Prüfung für Stammdaten (unbekannte Klassenstufen-/Lehrer-/Fach-Referenzen, unplausible Deputate, seit Phase 2.19 auch unbekannte Schüler-/Gruppen-Referenzen und doppelte Schüler-IDs), gleiche "Fail-Fast VOR jedem Solve"-Philosophie wie `Validation.vb`. | Stammdaten.vb |
